...
function updateSongInfoDropped(filename, subtune) {
  ...
  $("#song_info").html("FileName: " + getFileName(filename) + "<br>"
    + metadata.author + " - " + metadata.game
    + ", Track " + (parseInt(subtune) + 1) + " of " + subtune_count + tracks);
}
...
var emu;
var node;
function play_song(filename, subtune) { //TODO KDK: UI play 1
  if(node) {
    node.disconnect();
    node = null;
  }
...
function playMusicData(payload, subtune) { //TODO KDK: UI play 2
